...
#define HS_LIDAR_L64_BLOCK_SIZE                                                \
  (HS_LIDAR_L64_UNIT_SIZE * HS_LIDAR_L64_UNIT_NUM +                            \
...
#define HS_LIDAR_L20_BLOCK_SIZE                                                \
  (HS_LIDAR_L20_UNIT_SIZE * HS_LIDAR_L20_UNIT_NUM +                            \
   HS_LIDAR_L20_BLOCK_HEADER_AZIMUTH)
...
#define HS_LIDAR_L20_BLOCK_PACKET_BODY_SIZE                                    \
  (HS_LIDAR_L20_BLOCK_SIZE * HS_LIDAR_L20_BLOCK_NUMBER)
#define HS_LIDAR_L20_PACKET_TAIL_SIZE (22)
#define HS_LIDAR_L20_PACKET_SIZE                                               \
  (HS_LIDAR_L20_HEAD_SIZE + HS_LIDAR_L20_BLOCK_PACKET_BODY_SIZE +              \
   HS_LIDAR_L20_PACKET_TAIL_SIZE)
...
typedef struct {
  ...
  float
      utm_heading; // WGS84 detects object heading angle (angle with true north)
...
typedef struct PacketsBuffer_s {
  ...
  inline bool hasEnoughPackets() {
    return ((m_iterPush - m_iterCalc > 0) ||
            ((m_iterPush - m_iterCalc + 36000 > 0) &&
             (m_buffers.end() - m_iterCalc < 1000) &&
             (m_iterPush - m_buffers.begin() < 1000)));
  }
...
public:
  ...
  PandarGeneral_Internal(
      ...
      bool coordinate_correction_flag); // the default timestamp type is LiDAR
                                        // time
  ~PandarGeneral_Internal();
  ...
  */
  int DecodeUdpData(unsigned char *app_data_buff, int data_length,
...
private:
  ...
  int ParseL64Data(HS_LIDAR_L64_Packet *packet, const uint8_t *recvbuf,
                   const int len);
  int ParseL20Data(HS_LIDAR_L20_Packet *packet, const uint8_t *recvbuf,
                   const int len);
  int ParseQTData(HS_LIDAR_QT_Packet *packet, const uint8_t *recvbuf,
                  const int len);
  int ParseXTData(HS_LIDAR_XT_Packet *packet, const uint8_t *recvbuf,
                  const int len);
  ...
  void CalcL64PointXYZIT(HS_LIDAR_L64_Packet *pkt, int blockid,
                         char chLaserNumber,
                         boost::shared_ptr<PPointCloud> cld);
  void CalcL20PointXYZIT(HS_LIDAR_L20_Packet *pkt, int blockid,
                         char chLaserNumber,
                         boost::shared_ptr<PPointCloud> cld);
  void CalcQTPointXYZIT(HS_LIDAR_QT_Packet *pkt, int blockid,
                        char chLaserNumber, boost::shared_ptr<PPointCloud> cld);
  void CalcXTPointXYZIT(HS_LIDAR_XT_Packet *pkt, int blockid,
                        char chLaserNumber, boost::shared_ptr<PPointCloud> cld);
