...
void lidarCallback(boost::shared_ptr<PPointCloud> cld, double timestamp) {
  ...
  if (pcdFileWriteFlag == 1) {
    ...
    if (saveFrameIndex == frameItem) {
      ...
      for (int i = 0; i < Num; i++) {
        zos << cld->points[i].x << "," << cld->points[i].y << ","
            << cld->points[i].z << "," << cld->points[i].intensity << ","
            << cld->points[i].timestamp << "," << cld->points[i].ring
            << std::endl;
      }
    }
  }
...
void RunPcapPandarGeneralSDK(char *correctionFile, char *pcapFile,
                             char *lidarType, char *timestampType,
                             int runTime) {
  PandarGeneralSDK pandarGeneral(pcapFile, lidarCallback, 0, 0, 1, lidarType,
                                 "", timestampType, true);
...
void RunLidarPandarGeneralSDK(char *deviceipaddr, int lidarport, int gpsport,
                              char *correctionfile, char *lidarType,
                              char *timestampType, char *multiCastIp,
                              int runTime) {
  PandarGeneralSDK pandarGeneral(deviceipaddr, lidarport, 0, gpsport,
                                 lidarCallback, lidarAlgorithmCallback,
                                 gpsCallback, 0, 0, 1, lidarType,
                                 std::string("frame_id"), timestampType,
                                 correctionfile, multiCastIp, false);
  pandarGeneral.Start();
  sleep(runTime);
  return;
}
